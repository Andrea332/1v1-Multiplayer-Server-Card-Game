...
namespace Game
{
    [CreateAssetMenu(fileName = "Tsunami_Ability", menuName = "Cards/Ability/Tsunami")]
    public class Tsunami_Ability : BaseCardAbility
    {
        [SerializeField] private BaseItemString shipType;
        [SerializeField] private BaseItemString submarineType;
        ...
        public override void ServerDoAbility(CardBattleBoardManager cardBattleBoardManager, string cardEconomyId, string cardInventoryId, string serverJsonParameter)
        {
            ...
            playerTableInfo.cemeteryCardDatas.Add(playerTableInfo.qgSlotInfo.slotCardData);
            
            playerTableInfo.qgSlotInfo.slotCardData = null;
            
            foreach (var slotInfo in enemyTableInfo.slotInfos)
            {
                if(slotInfo.slotCardData == null) continue;

                if (slotInfo.slotCardData.type != shipType.ItemValue && slotInfo.slotCardData.type != submarineType.ItemValue) continue;
            ...
            TsunamyParameter parameter = new()
            {
        ...
        {
            TsunamyParameter parameter = JsonUtility.FromJson<TsunamyParameter>(jsonParameter);
        ...
        private struct TsunamyParameter
        {
